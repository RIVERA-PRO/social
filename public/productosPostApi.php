...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $estado = $_POST['estado'];
        $precio = $_POST['precio'];
        if (!empty($descripcion) && !empty($idUsuario) && !empty($categoria) && !empty($titulo) && !empty($estado) && !empty($precio)) {
            ...
            if ($imagenesPresentes) {
                ...
                $sqlInsert = "INSERT INTO `productos` (descripcion, idUsuario, categoria, titulo,estado,precio, imagen1, imagen2 , imagen3, imagen4) 
                              VALUES (:descripcion, :idUsuario, :categoria, :titulo,:estado,:precio, :imagen1, :imagen2, :imagen3 , :imagen4)";
                ...
                $sqlInsert = "INSERT INTO `productos` (descripcion, idUsuario, categoria, titulo,estado,precio) 
                              VALUES (:descripcion, :idUsuario, :categoria, :titulo,:estado,:precio)";
                $stmt = $conexion->prepare($sqlInsert);
                $stmt->bindParam(':descripcion', $descripcion);
                $stmt->bindParam(':idUsuario', $idUsuario);
                $stmt->bindParam(':categoria', $categoria);
